...
class Identicon {
	...
	tree() {
		...
		trunk.fillColor = `rgba(${trunkRGB[0]}, ${trunkRGB[1]}, ${trunkRGB[2]}, 0.5)`;
		...
		for (let level = 0; level < 6; level++) {
			...
			for (let i = 0; i < numLeaves; i++) {
				...
				if (shapeType == 0) {
					leaf = new paper.Shape.Circle(leafCenter, cellSize*this.s*shapeSize*0.9);
				} else if (shapeType == 1) {
					leaf = new paper.Path.RegularPolygon(leafCenter, 6, cellSize*this.s*shapeSize);
				} else if (shapeType == 2) {
					leaf = new paper.Path.RegularPolygon(leafCenter.add(new paper.Point(0, 1*this.s)), 3, cellSize*this.s*shapeSize*1.1);
				} else {
					leaf = new paper.Path.RegularPolygon(leafCenter, 5, cellSize*this.s*shapeSize);
				}
